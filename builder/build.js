...
if (!dir) {
  console.error('Использование: node builder/build.js <папка с .md> [index.json]');
...
for (const fileName of files) {
  const { data, content } = matter(await fs.readFile(path.join(dir, fileName), 'utf8'));
  ...
  for (const s of new Set(skips)) {
    problems.push(`${fileName}: «${s}» не поддерживается панелью и будет пропущен`);
  }
  index.push({
