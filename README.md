...
- Баннеры: папка `banners/` рядом с `.md` — картинка с именем документа
  (`about.md` → `banners/about.png|jpg|jpeg|webp`) автоматически становится
  заголовочной (`header`). Нет папки `banners/` — шаг пропускается. Варнинги: у
  страницы нет баннера, есть картинка без одноимённой страницы, у документа
  несколько картинок-баннеров.
